...
model = Sequential()
model.add(Dense(64,input_shape=(24,),activation='relu',kernel_regularizer = regularizers.l2(0.002)))
...
#（For the first 10001 steps, the learning rate is 1e-0, for the next 15000 steps the learning rate is 1e-1 and for any other steps the learning rate is 1e-2.
step = tf.Variable(0, trainable=False)
schedule = tf.optimizers.schedules.PiecewiseConstantDecay(
        [10000, 11000], [1e-0, 1e-1, 1e-2])
    # lr and wd be a function or a tensor
...
history = model.fit(scaled_train, train_label, epochs = 250, batch_size = 512,
                    validation_data=(scaled_validation, validation_label),
                   )
